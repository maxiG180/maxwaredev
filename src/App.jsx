...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#cbd5e1] font-sans antialiased selection:bg-white selection:text-black">
      ...
      {/* Increased Visibility Background Texture */}
      <div 
      ...
      <main className="container mx-auto px-8 md:px-16 text-left">
        ...
        <section className="min-h-screen flex flex-col justify-center pt-32 pb-20 text-left">
          <div className="grid lg:grid-cols-[1fr_auto] gap-24 items-center w-full reveal text-left">
            ...
            <div className="max-w-2xl py-10 text-left">
              ...
              <h2 className="text-xl md:text-2xl text-gray-300 font-light mb-10 tracking-widest uppercase">
              ...
              <p className="text-lg text-gray-400 max-w-xl mb-12 leading-relaxed font-extralight">
            ...
            <div className="hidden lg:flex flex-col gap-10 text-left">
              {/* Profile */}
              ...
              {/* Document Access Card with Live-ish Preview */}
              <div 
              ...
              >
                <div className="flex items-center justify-between mb-6 text-gray-500 font-light">
                ...
                <div className="flex items-center gap-6">
                  {/* Embedded PDF Thumbnail (Scaled iframe) */}
                  <div className="w-14 h-18 bg-[#111] rounded-lg border border-white/10 shadow-xl shrink-0 group-hover:border-white/30 transition-colors relative overflow-hidden pointer-events-none">
                    <iframe 
                      src="/resources/Maksym_CV_2_0.pdf#toolbar=0&navpanes=0&scrollbar=0" 
                      className="w-[400%] h-[400%] origin-top-left scale-[0.25] opacity-50 grayscale"
                      title="CV Thumbnail"
                    ></iframe>
                    <div className="absolute inset-0 bg-cyan-400/5 group-hover:bg-transparent transition-colors"></div>
                  </div>
                  <div>
                    <div className="text-sm font-light text-white uppercase tracking-[0.2em] leading-none mb-2 text-left">Technical CV</div>
                    <div className="text-[10px] text-gray-500 font-light uppercase tracking-widest italic text-left">Click to review full screen</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* CV Modal HD */}
        {isCvOpen && (
          <div className="fixed inset-0 z-[200] bg-[#000]/98 backdrop-blur-3xl flex items-center justify-center p-4 md:p-10 text-left text-left">
            <button 
            ...
            <div className="max-w-3xl space-y-24 text-left">
              ...
              <div className="space-y-12 text-left">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-10 border-b border-white/[0.03] pb-12 text-white text-left">
                  <div className="flex items-center gap-10 text-left text-left">
                    <div className="w-24 h-24 shrink-0 flex items-center justify-center transition-transform duration-500 hover:scale-110 p-2 bg-white/[0.02] rounded-xl border border-white/[0.05]">
                      <img src="/resources/fontyslogo.png" alt="Fontys" className="max-w-full max-h-full object-contain rounded-lg" />
                    </div>
                    <div>
                ...
                <div className="space-y-6 text-left text-left">
                  ...
                  <p className="text-gray-400 font-extralight leading-relaxed max-w-2xl text-lg italic text-left text-left">
              ...
              <div className="grid md:grid-cols-2 gap-16 pt-8 text-left text-left">
                <div className="p-10 bg-white/[0.01] border border-white/[0.03] rounded-[2.5rem] hover:bg-white/[0.02] transition-all group text-left">
                  <div className="flex items-center gap-6 mb-8 text-left text-left text-left">
                    <div className="w-20 h-20 flex items-center justify-center bg-white/[0.02] rounded-xl border border-white/[0.05] p-2 transition-transform group-hover:scale-105">
                      <img src="/resources/inetelogo.png" alt="INETE" className="max-w-full max-h-full object-contain grayscale opacity-80 group-hover:grayscale-0 transition-all" />
                    </div>
                    <h5 className="text-[9px] text-white uppercase font-light tracking-[0.4em]">INETE Lisbon</h5>
                  </div>
                  <p className="text-[10px] text-gray-400 uppercase tracking-[0.3em] font-extralight leading-loose italic text-left text-left">
                    IT & Software Development <br/> Professional programming <br/> 2020 — 2023
                  </p>
                </div>
                <div className="p-10 bg-white/[0.01] border border-white/[0.03] rounded-[2.5rem] hover:bg-white/[0.02] transition-all group text-left">
                  <div className="flex items-center gap-6 mb-8 text-left text-left text-left">
                    <div className="w-28 h-28 flex items-center justify-center bg-white/[0.02] rounded-xl border border-white/[0.05] p-2 transition-transform group-hover:scale-105">
                      <img src="/resources/assembly.png" alt="Assembly" className="max-w-full max-h-full object-contain grayscale opacity-80 group-hover:grayscale-0 transition-all" />
                    </div>
        ...
        {/* Experience Section */}
        <section id="experience" className="py-40 border-t border-white/[0.03] text-left text-left text-left text-left text-left">
            ...
            <div className="space-y-32 text-left">
              {[
                ...
                <div key={i} className="reveal group text-left text-left">
                  <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 border-b border-white/[0.03] pb-10 gap-8 text-left text-left">
                    <div className="flex items-center gap-10 text-left text-left text-left text-left">
                      <div className={`${exp.isLarge ? 'w-36 h-36' : 'w-24 h-24'} shrink-0 flex items-center justify-center transition-transform duration-500 hover:scale-110 bg-white/[0.02] rounded-2xl border border-white/[0.05] p-3`}>
                        <img src={exp.logo} alt={exp.company} className="max-w-full max-h-full object-contain rounded-lg" />
                      </div>
                      <div className="text-left text-left">
                        <h4 className="text-3xl font-light text-white tracking-[0.1em] uppercase leading-none text-left text-left">{exp.company}</h4>
                        <div className="text-xs text-gray-400 font-extralight uppercase tracking-[0.3em] mt-4 leading-none italic text-left text-left">{exp.role}</div>
                      </div>
                    </div>
                    <span className="text-[9px] font-light text-gray-400 uppercase tracking-[0.5em] shrink-0 italic text-right text-left text-left text-left">{exp.date}</span>
                  </div>
                  <p className="text-gray-400 text-lg mb-12 max-w-3xl leading-relaxed font-extralight text-left text-left">{exp.desc}</p>
                  <div className="flex flex-wrap gap-x-10 gap-y-5 text-[9px] font-normal text-gray-600 uppercase tracking-[0.3em] text-left text-left text-left">
                    {exp.tech.map(s => (
                      <span key={s} className="flex items-center gap-3 text-left">
                        <span className="w-1 h-1 bg-white/20 rounded-full text-left"></span>
                        {s}
                      </span>
                    ))}
                  </div>
        ...
        <section id="projects" className="py-40 border-t border-white/[0.03] text-left text-left text-left text-left">
          ...
          <div className="grid md:grid-cols-2 gap-px bg-white/[0.03] border border-white/[0.03] reveal rounded-[3rem] overflow-hidden shadow-2xl text-left text-left text-left text-left">
            {[
              {
              ...
              <div key={i} className="bg-[#050505] p-16 md:p-24 hover:bg-[#080808] transition-all group relative overflow-hidden text-left text-left text-left text-left">
                ...
                <div className="text-[9px] font-normal text-gray-500 uppercase tracking-[0.5em] mb-12 group-hover:text-white transition-colors italic text-left text-left text-left text-left">{p.client}</div>
                <h4 className="text-4xl font-normal text-white mb-8 tracking-widest uppercase leading-tight text-left text-left text-left">{p.title}</h4>
                <p className="text-gray-400 text-lg mb-12 font-extralight leading-relaxed italic text-left text-left text-left text-left text-left">{p.desc}</p>
                <div className="flex flex-wrap gap-10 text-[9px] font-light text-gray-600 uppercase tracking-[0.3em] text-left text-left text-left text-left">
                  {p.tags.map(t => <span key={t} className="group-hover:text-white transition-colors text-left text-left text-left text-left">{t}</span>)}
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Minimal Footer */}
        <footer className="py-32 border-t border-white/[0.03] flex flex-col md:flex-row justify-between items-center gap-12 text-white text-left text-left text-left">
          <div className="text-[9px] font-light text-gray-400 tracking-[0.6em] uppercase text-center md:text-left leading-loose italic text-left text-left text-left text-left">
            © 2026 MaxWare Protocol <br /> Architectural Design by Maksym Grebeniuk
          </div>
          <div className="flex gap-12 md:gap-16 text-left text-left text-left text-left text-left">
